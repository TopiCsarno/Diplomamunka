...
%----------
A generatív modelleket a statisztikai modellek osztályának tekintik, amelyek új adatpéldányokat hozhatnak létre. Ezeket a modelleket a felügyelet nélküli gépi tanulásban használják olyan feladatok elvégzésére, mint pl

% Valószínűség és valószínűség becslés,
% Modellezési adatpontok,
% A jelenség adatokban való leírásához,
% E valószínűségek alapján meg kell különböztetni az osztályokat.
% Mivel az ilyen típusú modellek gyakran a Bayes -tételre támaszkodnak, hogy megtalálják az együttes valószínűséget, a generatív modellek komplexebb feladatot is megoldhatnak, mint az analóg diszkriminatív modellek.

% Tehát a generációs modellek az egyes osztályok eloszlására összpontosítanak egy adathalmazban, és a tanulási algoritmusok hajlamosak modellezni az adatpontok mögöttes mintáit vagy eloszlását. Ezek a modellek a közös valószínűség fogalmát használják, és létrehozzák azokat az eseteket, amikor egy adott tulajdonság (x) vagy bemenet és a kívánt kimenet vagy címke (y) egyszerre léteznek.

% Ezek a modellek valószínűségi becsléseket és valószínűséget használnak az adatpontok modellezésére és az adathalmazban található különböző osztálycímkék megkülönböztetésére. A diszkriminatív modellektől eltérően ezek a modellek új adatpontok generálására is képesek.

% Mindazonáltal jelentős hátrányuk is van - Ha vannak kiugró értékek az adatkészletben, akkor ez jelentős mértékben befolyásolja az ilyen típusú modelleket.


%-----
% A generatív modellezés egy felügyelet nélküli tanulási feladat a gépi tanulásban, amely magában foglalja az új adatminták előállítását a képzési adatok valószínűségi eloszlásából. Bizonyos adatok ismeretében a cél az, hogy legyen egy modell az adatok mögöttes valószínűségi eloszlására, hogy mintákat vehessünk a képzési adatainkhoz.
...
% Mathematically, we think about a dataset of examples x_1, \ldots, x_nx 1 ​ ,…,x n ​ as samples from a true data distribution p(x)p(x). In the example image below, the blue region shows the part of the image space that, with a high probability (over some threshold) contains real images, and black dots indicate our data points (each is one image in our dataset). Now, our model also describes a distribution \hat{p}_{\theta}(x) p ^ ​ θ ​ (x) (green) that is defined implicitly by taking points from a unit Gaussian distribution (red) and mapping them through a (deterministic) neural network — our generative model (yellow). Our network is a function with parameters \thetaθ, and tweaking these parameters will tweak the generated distribution of images. Our goal then is to find parameters \thetaθ that produce a distribution that closely matches the true data distribution (for example, by having a small KL divergence loss). Therefore, you can imagine the green distribution starting out random and then the training process iteratively changing the parameters \thetaθ to stretch and squeeze it to better match the blue distribution.
% + ábra
